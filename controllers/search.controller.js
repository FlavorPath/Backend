...
exports.searchRestaurants = async (req, res) => {
    const { toggle, query, cursor = 0, limit = 10 } = req.query; // 클라이언트 요청에서 검색 기준(toggle), 검색 키워드(query), 커서(cursor), 그리고 가져올 데이터 수(limit)를 추출
    ...
    try {
        ...
        if (toggle === 'name') {
            ...
            `;
            params = [`%${query}%`, cursor, parseInt(limit)]; // SQL에 사용할 매개변수 설정
